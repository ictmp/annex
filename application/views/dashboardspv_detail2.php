                    ...
                    <div class="modal-body">
                        <section class="invoice">
                            ...
                            <div class="row no-print">
                                ...
                                <br><br>
                                <div id="attachfile" class="col-xs-12"></div>
                            </div>
                        </section>
                    </div>
...
<script>
    $(function () {
        $("#tblPackageList").dataTable({
            "ajax": {
                "url": "<?php echo site_url('detail2list'); ?>",
                "type": "POST",
                   ...
                   $('#attachfile').html("" +
                       "<div class='col-md-3'>" +
                            ...
                            "<div class='btn-group btn-group-justified' role='group'>" +
                                "<a href='<?php echo site_url('downloadfile/'); ?>" + value['fileid'] + "' class='btn btn-sm btn-success'>Download</a>" +
                            "</div>" +
                       "</div>");
                });
    ...
    function btnApproval(approval) {
        $.post("<?php echo site_url('detail2action'); ?>", {
            'approval': approval,
            'companyid': $('#companyid_preview').text(),
            'packageid': $('#packageid_preview').text()
        }, function (data) {
            if(data.errmsg == "-") {
                if(approval == "1") {
                    toastSuccess('MCU Package succesfully approved. This page will automatically redirected in few second.','');
                    setTimeout("redirectPage()", 1300);
                } else {
                    setTimeout("redirectPage()", 500);
                }

            } else {
                toastError(data.errmsg,''); return false;
            }
        }, "json");
    }

    function redirectPage() {
        window.location = "<?php echo site_url('mainpage'); ?>";
    }
</script>
